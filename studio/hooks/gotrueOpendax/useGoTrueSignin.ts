...
export const useGoTrueSignin = (
  ...
  const sendChallengeToken = (token: string) => {
    axios
      .post(
        `${process.env.NEXT_PUBLIC_GOTRUE_URL}/asymmetric_login`,
        {
          key: account,
      ...
      .catch((err) => {
        if (process.env.NEXT_PUBLIC_IS_DEV_MODE && err.code === 401) {
  ...
  const signChallenge = () => {
    axios
      .post(
        `${process.env.NEXT_PUBLIC_GOTRUE_URL}/sign_challenge`,
        {
          key: account,
          algorithm: 'ETH',
        },
