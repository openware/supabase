...
const handlePost = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  const response = await post(`${PG_META_URL}/schemas`, payload, {
    headers,
  })
...
const handlePatch = async (req: NextApiRequest, res: NextApiResponse) => {
  const headers = constructHeaders(req.headers)
  const payload = toSnakeCase(req.body)
  const response = await patch(`${PG_META_URL}/schemas/${req.query.id}`, payload, {
    headers,
  })
...
const handleDelete = async (req: NextApiRequest, res: NextApiResponse) => {
  const headers = constructHeaders(req.headers)
  const response = await delete_(`${PG_META_URL}/schemas/${req.query.id}`, {}, { headers })
