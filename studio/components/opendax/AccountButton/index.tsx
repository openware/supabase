  ...
  if (!user?.id || !active) {
    return (
      <>
        ...
        <ConnectorWalletModal showModal={showModal} handleModal={setModal} />
      </>
    )
  }
    ...
    <>
      ...
      >
        <div className={itemsAlignmentClassNames}>
          ...
          {!minimized && (
            <div className={classnames('ml-2 truncate', contentClassNames)}>
              <div className={chainClassNames}>
                <p className={chainTextClassNames}>{ENSName || `${shortenHex(account, 4)}`}</p>
                {chain && <div className={chainNetworkClassNames}>{chain.network}</div>}
              </div>
            </div>
          )}
        </div>
...
AccountButtonWidget.defaultProps = {
  buttonClassNames:
    'inline-flex w-full justify-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-primary-cta-layer-color-60 duration-200 bg-primary-cta-color-60 hover:bg-primary-cta-color-80 active:bg-primary-cta-color-90',
  buttonActiveClassNames:
    'flex justify-center lg:justify-start p-2 group rounded-md hover:bg-navbar-control-bg-color-10',
  chainNetworkClassNames:
    'ml-1 px-2 py-1 text-xs font-medium uppercase rounded-full bg-secondary-cta-color-10 text-secondary-cta-color-90',
  chainClassNames: 'flex items-center text-base font-bold text-gray-700 group-hover:text-gray-900',
  chainTextClassNames:
    'text-xs font-medium truncate text-neutral-control-layer-color-100 font-variant-none',
  itemsAlignmentClassNames: 'flex items-center',
  iconClassNames:
    'flex h-8 w-8 items-center justify-center bg-navbar-control-bg-color-10 rounded-full',
  connectWalletButtonIcon: (
    <span>
      <WalletIcon classNames="h-5 w-5" stroke="var(--primary-cta-layer-color-60)" />
    </span>
  ),
}
